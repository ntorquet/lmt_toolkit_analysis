...
def plotActivityPerAnimalWholeExperiment(file, timeBin):
    ...
    for animal in pool.animalDictionnary.keys():
        ...
        dt[animal] = pool.animalDictionnary[animal].getDistancePerBin(binFrameSize=timeBin * oneMinute, minFrame=tmin, maxFrame=tmax)
...
def extractActivityPerAnimalWholeExperiment(file, timeBin):
    ...
    for animal in pool.animalDictionnary.keys():
        ...
        activity[rfid] = pool.animalDictionnary[animal].getDistancePerBin(binFrameSize=timeBin * oneMinute, minFrame=tmin, maxFrame=tmax)
        totalDistance[rfid] = pool.animalDictionnary[animal].getDistance(tmin=tmin, tmax=tmax)
...
def orderDataByTreatmentFromListOfPool(listOfPool, tmin, tmax, timeBin):
    ...
    for animalPool in listOfPool:
        for animal in animalPool.getAnimalDictionnary():
            animalData = animalPool.animalDictionnary[animal]
            ...
            print(activity)
            print(animalPool.animalDictionnary[animal].user1)
    return output
